(function (Drupal, drupalSettings, once) {
  ...
  Drupal.maveLoadComponents = function () {
    const settings = drupalSettings.mave || {};
    if (!componentsPromise) {
      window.__maveComponentsConfig = settings.componentsConfig || {};
      componentsPromise = import(settings.componentsSrc).then((module) => {
        if (module.configureMave) module.configureMave(settings.componentsConfig || {});
        return module;
      }).catch((error) => {
  ...
  Drupal.behaviors.mavePlayer = {
    attach(context) {
      ...
      if (once('mave-player', 'mave-player, [data-mave-editor]', context).length) {
