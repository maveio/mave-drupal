(function maveDialog(Drupal, $) {
  function syncDialogs() {
    document
      .querySelectorAll('.ui-dialog:has(.mave-library-add-form)')
      .forEach((dialog) => {
        ...
        const button = dialog.querySelector(
          '.ui-dialog-buttonpane .mave-continue',
        );
        ...
        if ($(content).data('ui-dialog')) {
          $(content).dialog(
            ...
            'title',
            picker
              ? Drupal.t('Select a Mave video')
              : Drupal.t('Video details'),
          );
        }
        ...
        selection.textContent =
          picker.dataset.selectionLabel || Drupal.t('No video selected');
        selection.title = selection.textContent;
      });
  }

  Drupal.behaviors.maveDialog = {
    attach: () => requestAnimationFrame(syncDialogs),
  };
  window.addEventListener('dialog:aftercreate', syncDialogs);
  document.addEventListener('dialogButtonsChange', () =>
    requestAnimationFrame(syncDialogs),
  );
